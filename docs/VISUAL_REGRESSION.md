...
The 0.4.0 formal-test functional baseline was generated by GitHub Actions run
`30408544276`, evidence job `90439382718`, from exact source revision
`82b1abc1b921b37c730e54c699574272c54e8f0e`. Its candidate manifest SHA-256
is `9e132dc5fdff0fdd9fcc14823f3c82db9fbcfbcc619faa1749853c94c1a0c4c6`.
Twenty-six PNGs are byte-identical to the preceding accepted baseline. The 24
changed images are the Light and Dark variants of Local Media ready/working,
onboarding empty, Recording active/loading/ready, all four shell sizes, and
Transcript incomplete/selected. These changes record only the authorized v4
functional additions: the Start a New Meeting coordinator, Codex Assistant
navigation, explicit Codex-text permission, independent speech-to-text route
and readiness, and transcript outline/search controls. They do not begin the
deferred U1 visual redesign.

All 50 candidate images were reviewed in labeled Light and Dark contact sheets.
The changed Local Media, Recording, onboarding, narrow/expanded shell, and
Transcript inspector states were also inspected at full resolution. The review
found no blank capture, overlap, clipping, missing primary action, or
privacy-bearing content. Candidate structure, descriptors, environment,
manual-system cases, filenames, and all 50 declared PNG hashes match the
accepted contract. Every comparison threshold remains exact:
`maximumChannelDelta = 0`, `maximumChangedPixelRatio = 0`, and
`minimumLuminanceSSIM = 1`.

The matching 0.4.0 calibration was generated by GitHub Actions run
`30409045361`, evidence job `90440892621`, from the same exact source and
environment. Three fresh processes each captured five continuously hosted
frames for all 50 cases: 750 frames and 1,500 within-process pair
relationships. Every hash was identical within and across processes, and every
hash matched the corresponding accepted candidate PNG. All measured
differences were zero. The durable records are
`docs/audits/visual-regression-calibration/calibration-1.json` through
`calibration-3.json`.

As with the preceding accepted baseline, each calibration record intentionally
binds the pre-import committed manifest SHA-256
`63fac4d34e01baa4e48b30e9420c313066fe2e6b21f237defa2d5ef3ba2e561c`,
while its per-case capture hashes match the reviewed candidate manifest
`9e132dc5fdff0fdd9fcc14823f3c82db9fbcfbcc619faa1749853c94c1a0c4c6`.
The evidence is therefore tied to the exact pre-import source rather than
rewritten after the baseline carrier commit.
