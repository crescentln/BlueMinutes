...
Status: pre-publication evidence map; refresh on the exact public commit
...
| Is quality actively maintained? | [CI workflow](../.github/workflows/ci.yml), [execution state](CODEX_EXECUTION_STATE.md), and [Tests](../Tests/) | The current local warning-as-error gate passes a 248-test suite in 43 suites; three installed-model routes remain explicit opt-in checks rather than ordinary-CI claims. |
...
These are local pre-publication results. Replace them with the exact public
commit SHA and successful GitHub Actions run before applying to any program.

Reachable `main` history has completed its private-path sanitization. Local
Codex checkpoint refs and reflogs remain rollback-only state: initial
publication must push only `refs/heads/main:refs/heads/main`, must never use
`--mirror`, and must never publish or share the repository's `.git` directory.
Historical audit documents may retain pre-sanitization commit IDs where they
are explicitly labeled as immutable snapshots.
